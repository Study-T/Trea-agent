...
**Daily work logs are stored in `context-log/logs/` — ONE FILE PER DAY!**
**日常工作日志存储在 `context-log/logs/`——每天一个新文件！**

Example:
- `context-log/logs/2026-04-19.md`
- `context-log/logs/2026-04-20.md`
- `context-log/logs/2026-04-21.md`

See `context-log/SKILL.md` for full details on daily logging.
详细的每日日志记录规则请参阅 `context-log/SKILL.md`。
...
### PM Phase Complete
### PM阶段完成
- **Trigger**: PM Leader completes review AND Boss approves
- **触发**: PM组长完成审核 **且** Boss通过审核
- **Recorder Action**: Record to today's `context-log/logs/YYYY-MM-DD.md` immediately
- **记录者动作**: 立即记录到今天的 `context-log/logs/YYYY-MM-DD.md`

### RD Phase Complete
### RD阶段完成
- **Trigger**: RD Leader completes review AND Boss approves
- **触发**: RD组长完成审核 **且** Boss通过审核
- **Recorder Action**: Record to today's `context-log/logs/YYYY-MM-DD.md` immediately
- **记录者动作**: 立即记录到今天的 `context-log/logs/YYYY-MM-DD.md`

### QA Phase Complete
### QA阶段完成
- **Trigger**: QA Leader completes review AND Boss approves
- **触发**: QA组长完成审核 **且** Boss通过审核
- **Recorder Action**: Record to today's `context-log/logs/YYYY-MM-DD.md` immediately
- **记录者动作**: 立即记录到今天的 `context-log/logs/YYYY-MM-DD.md`

### Requirement Complete
### 需求完成
- **Trigger**: All three phases (PM → RD → QA) have passed Boss approval
- **触发**: 三个阶段（PM → RD → QA）全部通过Boss审核
- **Recorder Action**: Record to today's `context-log/logs/YYYY-MM-DD.md`
  - Write phase records in order (PM → RD → QA)
  - Put complete requirement summary at the BOTTOM of the log
- **记录者动作**: 在今天的 `context-log/logs/YYYY-MM-DD.md` 记录
  - 按顺序记录阶段（PM → RD → QA）
  - 把完整需求总结放在日志的**最下面**
...
```
requirements/                          ← Templates and protocol / 模板和协议
...
context-log/                          ← Daily logs / 每日日志
└── logs/
    ├── 2026-04-19.md              ← One file per day / 每天一个文件
    ├── 2026-04-20.md              ← Separate file / 独立文件
    └── ...
```
...
See `context-log/SKILL.md` for full format and rules.
详细格式和规则请参阅 `context-log/SKILL.md`。
