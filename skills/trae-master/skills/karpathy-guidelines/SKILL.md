...
**Principle / 原则**: Strictly prohibit speculating, judging, inferring, or predicting any false information. All data must be real.
严禁根据任何东西推测、判断、推断、预测等虚假信息，所有的数据必须是真实的。

### Practical guidance / 实践指导：

- Never fabricate data when it is missing / 数据缺失时严禁编造
- Never infer or predict values without real evidence / 没有真实证据时严禁推断或预测数值
- Never guess API responses, database records, or configuration values / 严禁猜测API响应、数据库记录或配置值
- When uncertain, explicitly state "unverified, needs confirmation" / 不确定时明确标注"未验证，需确认"
- When data is missing, stop and request real data / 数据缺失时停下来请求真实数据

### Ask yourself / 问问自己：
> "Is every piece of data I'm using based on a real, verifiable source?"
> "我使用的每一项数据是否都基于真实、可验证的来源？"

If not, stop and get real data. / 如果不是，停下来获取真实数据。
...
| Data Authenticity | All data must be real, no speculation or fabrication / 所有数据必须真实，严禁推测或编造 |
...
See `knowledge/methods/karpathy-guidelines-examples.md` for real-world code examples demonstrating these principles.
有关展示这些原则的实际代码示例，请参阅 `knowledge/methods/karpathy-guidelines-examples.md`。
