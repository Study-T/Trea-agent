---
name: "context-log"
description: "Automatic context summary recording system. Triggered after user confirmation; records useful decisions/outputs/risks/next actions. One file per day."
---
...
This is the automatic context recording system. After Boss/Leaders finish the current work report and the user confirms the output is acceptable, it automatically summarizes and records USEFUL information (decisions, outputs, risks, next actions) and avoids noise.
这是自动上下文记录系统。在 Boss/组长汇报完当前工作并且用户确认输出可接受后，它会自动总结并记录有用信息（决策、产出、风险、下一步行动），不记录无效噪声。
...
|--------------|-----------------|
| Daily work logs | core/context-log/logs/YYYY-MM-DD.md |
| 每日工作日志 | core/context-log/logs/YYYY-MM-DD.md |
| Requirement records root | skills/workflow-engine/requirements/active/ |
| 需求记录根目录 | skills/workflow-engine/requirements/active/ |
| Completed records root | skills/workflow-engine/requirements/completed/ |
| 已完成记录根目录 | skills/workflow-engine/requirements/completed/ |
| Requirement-specific artifacts | skills/workflow-engine/requirements/[active|completed]/REQ-YYYY-NNN/ (created when a requirement is instantiated) |
| 需求专属产物 | skills/workflow-engine/requirements/[active|completed]/REQ-YYYY-NNN/（在需求实例化时创建） |
...
1. **Active requirements** → `skills/workflow-engine/requirements/active/`
   活跃需求 → 工作流引擎的需求活跃目录

2. **Completed requirements** → `skills/workflow-engine/requirements/completed/`
   已完成需求 → 归档到工作流引擎的需求完成目录

3. **Daily logs** → `core/context-log/logs/YYYY-MM-DD.md`
   每日日志 → 按日期分离
...
**Record ONLY after Boss/Leaders finish the current work report AND user confirms "acceptable" / 只有在 Boss/组长汇报完当前工作且用户确认"可以"后才记录**
**Exception: failure/abort records may be written after Boss confirms the workflow cannot proceed. / 例外：失败/终止记录在 Boss 确认工作流无法继续后即可写入。**
