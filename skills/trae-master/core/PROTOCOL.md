...
|-----------|---------|--------|
| Daily work logs | core/context-log/logs/YYYY-MM-DD.md | All skills |
| Requirement records (active) | skills/workflow-engine/requirements/active/ | All skills |
| Requirement records (completed) | skills/workflow-engine/requirements/completed/ | All skills |
| Per-requirement artifacts | Created by workflow when a requirement is instantiated | Phase-specific access |
...
- Active requirements → `skills/workflow-engine/requirements/active/`
- Completed requirements → `skills/workflow-engine/requirements/completed/`
- When a concrete requirement is created, the workflow **MUST create** a requirement-specific directory such as `skills/workflow-engine/requirements/active/REQ-YYYY-NNN/`
- 当创建具体需求时，工作流**必须创建**如 `skills/workflow-engine/requirements/active/REQ-YYYY-NNN/` 的需求专属目录
- The requirement-specific directory is the sole canonical storage location for that requirement's artifacts.
- 需求专属目录是该需求所有产物的唯一合法存储路径。
